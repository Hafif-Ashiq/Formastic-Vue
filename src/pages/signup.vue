...
const handleSignup = async () => {
  ...
  if (isLoading.value) {
    ...
    if (password.length < 8) {
        notyf.error("Password must be 8 or more characters")
        
    }
    else if (password !== c_password){
      notyf.error("Password and Confirm Password doesn't match")
  ...
  // // formData.append('profile_pic',fileInput.value)
  console.log('Data:');
  console.log(data);
  ...
  await axios.post(url + 'register/',data,
  {
    ...
    if (response.status === 201) {
      ...
      step.value++
      notyf.dismissAll()
      notyf.success('Welcome, ' + first_name + " " + last_name)
  ...
  isLoading.value = true
    ...
    if(isLoading.value){
      ...
      if (step.value === 0) {
        if (first_name == '') {
          notyf.dismissAll()
          notyf.error("First Name can not be empty")
          isLoading.value = false
          return
        }
        if (last_name == '') {
          notyf.dismissAll()
          notyf.error("Last Name can not be empty")
          isLoading.value = false
          return
        }
        if (last_name == '') {
        ...
        if (email == '') {
          notyf.dismissAll()
          notyf.error("Email can not be empty")
          isLoading.value = false
          return
        }
        else{
        ...
        await axios.get(url + 'verify-email',{params:{
          'email' : email
        }        
        }).then(response => {
        ...
        if (fileInput.value) {
          ...
          notyf.error('Upload a Valid File')
        }
          ...
          <div class="container">
                  ...
                  >
                    ...
                    <div class="button-wrap has-help">
                      <VButton
                        ...
                        rounded
                        :loading="isLoading"
                        @click.prevent="handleStepContinue"
                      >
                        Continue
                      </VButton>
                      <span>
            ...
            >
              <div class="column is-4 is-offset-4 username-form">
                ...
                <h2 class="subtitle signup-subtitle has-text-centered">
                  Your Password is needed to have a secure and reliable account for your use.
